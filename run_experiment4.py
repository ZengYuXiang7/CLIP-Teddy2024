...
from modules.models.Retrieve import IVFPQIndex, HNSWIndex
...
def high_speed_retreive(database, query, model, k):
    ...
    if model == 'L2':
        ...
        print('执行PQ检索部分')
        t1 = time()
        lsh = IVFPQIndex(k=k, d=d, nlist=10, m=16)
        topk_distence, topk_indices = lsh.search_topk_embeds(database, query)
        t2 = time()
        print(f'PQ: {t2 - t1 : .2f}s')
    elif model == 'HNSW':
        print('执行HNSW检索部分')
        t1 = time()
        lsh = HNSWIndex(k=k, d=d)
        topk_distence, topk_indices = lsh.search_topk_embeds(database, query)
        t2 = time()
...
def retrieve_experiment(retrieve_method, ):
    # 图像检索文本
    print('-' * 80)
    print('图像检索文本')
...
if __name__ == '__main__':
    ...
    retrieve_experiment('L2')
    retrieve_experiment('LSH')
    retrieve_experiment('HNSW')
    retrieve_experiment('PQ')
    retrieve_experiment('KNN')
